...
if len(sys.argv) < 5:
    print("Usage: python3 run_torch.py <foreward file> <backward file> <number of steps> <batch size>")
    exit(1);
out_forward_file = sys.argv[1]
out_backward_file = sys.argv[2]
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input_batch, target) in enumerate(train_loader):
        ...
        with torch.autograd.profiler.profile(use_cuda=False) as f_prof:
            ...
            p = Popen(['bash', 'poll.sh', './{}_{}.mem_trace'.format(out_forward_file, count)])
        ...
        with torch.autograd.profiler.profile(use_cuda=False) as prof:
            # with torch.autograd.profiler.emit_nvtx():
            p = Popen(['bash', 'poll.sh', './{}_{}.mem_trace'.format(out_backward_file, count)])
            loss.backward()
            p.terminate()
